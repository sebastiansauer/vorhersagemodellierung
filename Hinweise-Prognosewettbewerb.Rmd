...
Ja, das keine doofe Antwort, aber man kann es besser machen. 
...
1.  Bei den Trainingsdaten liegen sowohl die erklärenden Variablen ${\bf{x}} = (x_1, x_2, \ldots, x_n)$ als auch die Zielvariable $y$ vor. 
   Auf diesen Trainingsdaten wird das Modell $y=f({\bf{x}})+\epsilon = f(x_1, x_2, \ldots, x_n)+\epsilon$ gebildet und durch $\hat{f}(\cdot)$ geschätzt. 

2.  Dieses geschätzte Modell ($\hat{f}(\cdot)$) wird auf die Anwendungsdaten $\bf{x}_0$, für die (Ihnen) die Werte der Zielvariable unbekannt sind, angewendet, d.h., es wird $\hat{y}_0 :=\hat{f}({\bf{x}}_0)$ berechnet. 
...
# Koeffizient zur Güte der Vorhersagegüte 
...
Zur Bewertung der Vorhersagegüte wird $R^2$ herangezogen.

Von zwei Prognosemodellen zum gleichen Datensatz ist dasjenige Modell besser,
das einen höheren $R^2$-Wert aufweist. Kurz gesagt: Höher ist besser.
...
- Zur Vorhersagegüte: Die Vorhersagegüte des Nullmodells entspricht einer $4,0$, die eines (unbekannten) einfachen Referenzmodells Is Dozentis einer $2,0$^[Gender-I: <https://gender-i.de/>].
...
12. In der Analyse muss als Ausgangspunkt der vom/von der Dozenten/in bereitgestellten Datensatz genutzt werden. Dazu muss der Datensatz zu Beginn des Skripts von der entsprechenden Internetadresse heruntergeladen werden.
